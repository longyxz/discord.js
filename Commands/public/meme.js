...
module.exports = {
    ...
    .setName("meme")
    .setDescription("nhận 1 meme")
    .addStringOption(option=>
        option.setName("platform")
        .setDescription("Pha Meme (Tùy chọn)")
        .addChoices(
    ...
    async execute(interaction){
        ...
        async function redditMeme(){
            fetch("https://www.reddit.com/r/memes/random/.json").then(async res=>{
                ...
                return interaction.reply({embeds: [redditEmbed]})
            })
        }
        // async function giphyMeme(){
        //     await fetch('https://').then(async res=>{
        //         let meme = await res.json()
